...
//
// The Saviynt style guide has regex patterns like:
//
...
func TestSpectral_RegexEscaping_FromJSON(t *testing.T) {
	// This JSON mimics the exact content of saviynt-api-style-guide.json SAV-012
	profileJSON := `{
		"name": "test-escaping",
		"version": "1.0.0",
		"rules": [
	...
	tests := []struct {
		...
		{
			name:     "SAV-012 kebab-case regex has correct escaping",
			ruleID:   "sav-012",
			contains: `\/[a-z]`,
			absent:   `\\\\/[a-z]`,
		},
		{
			name:     "SAV-007 semver regex has correct dot escaping",
			ruleID:   "sav-007",
			contains: `[0-9]+\\.[0-9]+\\.[0-9]+`, // YAML `\\.` = regex `\.` (escaped dot) — CORRECT
			absent:   `[0-9]+\\\\.[0-9]+`,        // Quadruple-backslash would be wrong
		},
		{
			name:     "SAV-013 trailing slash regex has correct escaping",
			ruleID:   "sav-013",
			contains: `\/$`,
			absent:   `\\\\/$`,
		},
